...
router.get('/test', async (req, res) => {
  try {
    console.log('🧪 Analytics test endpoint called (FIXED version)');
    ...
    res.json({ 
      success: true, 
      message: 'Analytics API is working with FIXED database queries!',
...
// GET /api/analytics/overview - Dashboard overview stats (FIXED)
router.get('/overview', allowAll, async (req, res) => {
  try {
    console.log('📊 Fetching FIXED analytics overview...');
    ...
    console.log('✅ FIXED Analytics overview:', overview);
    res.json({ success: true, data: overview });

  } catch (error) {
    console.error('❌ Error fetching fixed analytics overview:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/sales-chart - FIXED Sales data for charts
router.get('/sales-chart', allowAll, async (req, res) => {
  try {
    const { period = '7days' } = req.query;
    console.log(`📈 Fetching FIXED sales chart data for period: ${period}`);
    ...
    console.log(`✅ FIXED Sales chart data fetched: ${salesData.length} data points`);
    res.json({ success: true, data: salesData || [] });

  } catch (error) {
    console.error('❌ Error fetching fixed sales chart:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/top-products - FIXED Best selling products
router.get('/top-products', allowAll, async (req, res) => {
  try {
    console.log('🏆 Fetching FIXED top products...');
    ...
    console.log(`✅ FIXED Top products: ${topProducts.length} products`);
    res.json({ success: true, data: topProducts || [] });

  } catch (error) {
    console.error('❌ Error fetching fixed top products:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/customer-behavior - FIXED Customer behavior analytics
router.get('/customer-behavior', allowAll, async (req, res) => {
  try {
    console.log('👥 Fetching FIXED customer behavior analytics...');
    ...
    console.log('✅ FIXED Customer behavior analytics fetched');
    res.json({ success: true, data: behaviorData });

  } catch (error) {
    console.error('❌ Error fetching fixed customer behavior:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/chatbot-insights - FIXED AI chatbot conversation insights
router.get('/chatbot-insights', allowAll, async (req, res) => {
  try {
    console.log('🤖 Fetching FIXED chatbot insights...');
    ...
    console.log('✅ FIXED Chatbot insights fetched');
    res.json({ success: true, data: chatbotData });

  } catch (error) {
    console.error('❌ Error fetching fixed chatbot insights:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/inventory-performance - FIXED Inventory performance reports
router.get('/inventory-performance', allowAll, async (req, res) => {
  try {
    console.log('📦 Fetching FIXED inventory performance...');
    ...
    console.log('✅ FIXED Inventory performance fetched');
    res.json({ success: true, data: inventoryData });

  } catch (error) {
    console.error('❌ Error fetching fixed inventory performance:', error);
    res.status(500).json({ 
      success: false, 
...
// GET /api/analytics/real-time - FIXED Real-time dashboard data
router.get('/real-time', allowAll, async (req, res) => {
  try {
    console.log('⚡ Fetching FIXED real-time data...');
    ...
    console.log('✅ FIXED Real-time data fetched');
    res.json({ success: true, data: realTimeData });

  } catch (error) {
    console.error('❌ Error fetching fixed real-time data:', error);
    res.status(500).json({ 
      success: false, 
